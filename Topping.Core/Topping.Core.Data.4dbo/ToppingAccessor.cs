...
namespace Topping.Core.Data.Db4o
{
    ...
    public class ToppingAccessor : IDisposable
    {
        ...
        public List<GamesDetailDto> GetGamesDetail(string player, int year, int mont, Guid configId)
        {
            ...
            list = this.entity.Cast<GamesDetail>().Where<GamesDetail>(p => ((((p.Date > start) && (p.Date < end)) && (p.UserId == pid)) && (p.GameConfigId == configId))).ToList<GamesDetail>();
        ...
        public List<RankingDto> GetRankings(string player, int year, int mont, Guid configId)
        {
            ...
            {
                list = this.entity.Cast<GameRanking>().Where<GameRanking>(p => (((p.Playerid == pid) && (p.Year == year)) && (p.Month == mont))).ToList<GameRanking>();
            }
            if ((player.Length > 0) && (configId != Guid.Empty))
            {
                list = this.entity.Cast<GameRanking>().Where<GameRanking>(p => ((p.Playerid == pid) && (p.ConfigGameId == configId))).ToList<GameRanking>();
            }
